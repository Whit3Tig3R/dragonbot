...
--vardump(extra)
      ...
      elseif get_cmd == 'ban' then
        ...
        send_large_msg(receiver, 'User @'..member..' ['..member_id..'] بن شد :|')
        return ban_user(member_id, chat_id)
      elseif get_cmd == 'unban' then
        send_large_msg(receiver, 'User @'..member..' ['..member_id..'] انبن شد :|')
        local hash =  'banned:'..chat_id
        redis:srem(hash, member_id)
        return 'User '..user_id..' unbanned'
      elseif get_cmd == 'banall' then
        send_large_msg(receiver, 'User @'..member..' ['..member_id..'] سوپر بن شد')
        return banall_user(member_id, chat_id)
      elseif get_cmd == 'unbanall' then
        send_large_msg(receiver, 'User @'..member..' ['..member_id..'] از سوپر بن در اومد :|')
...
local function run(msg, matches)
 if matches[1]:lower() == 'id' then
    ...
    elseif matches[1]:lower() == 'id' then
      local name = user_print_name(msg.from)
      savelog(msg.to.id, name.." ["..msg.from.id.."] used /id ")
      return "ایدی گروه برای " ..string.gsub(msg.to.print_name, "_", " ").. ":\n\n"..msg.to.id  
    end
  end
  ...
  if matches[1]:lower() == 'ban' then-- /ban 
    if type(msg.reply_id)~="nil" and is_momod(msg) then
      ...
      if string.match(matches[2], '^%d+$') then
        ...
        local name = user_print_name(msg.from)
        savelog(msg.to.id, name.." ["..msg.from.id.."] یوزر بن شد ".. matches[2])
...
if matches[1]:lower() == 'kick' then
      ...
      if string.match(targetuser, '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then
        	...
        	banall_user(targetuser)
       		return 'User ['..user_id..' ] سوپر بن شد :-D'
      else
	local cbres_extra = {
		chat_id = msg.to.id,
		get_cmd = 'banall',
    ...
    local chat_id = msg.to.id
      if string.match(matches[2], '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then 
        	...
        	return 'User ['..user_id..' ] از سوپر بن در اومد :|'
      else
	local cbres_extra = {
		chat_id = msg.to.id,
		get_cmd = 'unbanall',
  ...
  if matches[1]:lower() == "gbanlist" then -- لیست سوپر بن ها
    return banall_list()
  end
end
